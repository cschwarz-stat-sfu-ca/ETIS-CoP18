...
#
#path.wkg <- 'C:/ETIS/analysis/Processed Data'   ## CJS
#path.code <- 'C:/ETIS/analysis/R Code'          ## CJS
#______________________________________________________________________
...
setwd(path.Rcode)          ## CJS changed to standard path name
source('sz id remove weight pieces model.R')

setwd(path.wkg)
df.szrecs <- read.csv('ivory sz recs 1900_2100_tidy.csv', header=TRUE)  # now wants _tidy; use TRUE/FALSE
...
df.raw <- df.szrecs %>%
  filter(raw) %>%
  dplyr::select(sz.id:raw.wgt)   ## CJS  add dplyr::
