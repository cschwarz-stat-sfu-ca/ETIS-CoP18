...
#
#year.from <- 2007    ## CJS  remove from this file as this file needs to be run twice
#year.to <- 2017      ## CJS  remove from this file as this file needs to be run twice
...
#
#path.Rcode <- 'C:/ETIS/analysis/R Code'          ## CJS - converted to comment and should be set outside of code fragment
#path.data <- 'C:/ETIS/analysis/Processed Data'   ## CJS
...
df.szrecs.tidy <- df.szrecs %>%
  ...
  filter(status.id >= statusMin)# %>%  # not sure why you need the select(~status.id)
#  dplyr::select(status.id)  ## CJS new format for select in dplyr - remove the ~ ; add dplyr::
...
df.szrecs.tidy <- df.szrecs.tidy %>%
  ...
  mutate(wkd.pres = replace(wkd.pres, wkd.pres == F, NA))# %>%  ## CJS now sure why you need the select here?
