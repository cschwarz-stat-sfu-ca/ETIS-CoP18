...
#
#year.crit.from <- 2008    ## CJS changed to year.crit.to since deals with seizeures.. Set outside code
#year.crit.to <- 2017
...
#
#path.code <- 'C:/ETIS/analysis/R code'         ## CJS set outside code block
...
df.raw <- df.sz.info %>%
  ...
  dplyr::select(id:raw_weight) %>%    ## CJS added dplyr::
...
df.raw <- df.raw %>%
  ...
  dplyr::select(id:discovered_country_id, r.gp = raw.wt)  ## CJS added dplyr::  raw_Wt changed to raw.wt

df.wkd <- df.sz.info %>%
  filter(!is.na(worked_pieces)|!is.na(worked_weight)) %>%
  dplyr::select(id:discovered_country_id, wkd_pieces = worked_pieces, wkd_weight = worked_weight) %>%
...
df.wkd <- df.wkd %>%
  ...
  dplyr::select(id:discovered_country_id, w.gp = wkd_wt)
...
for (i in 1:8){
  list.rw[[i]] <- list.rw[[i]] %>%
    filter((seizure_year.x >=  year.crit.from),   ## CJS year.from changed to year.crit.from
           (seizure_year.x <=  year.crit.to),     ## CJS year.to   changed to year.crit.to
...
nyr <- year.crit.to - year.crit.from + 1          ## CJS changed to year.to to year.crit.to etc
# ids for all ctries 
...
df.ctry.used <-  df.ctry %>%
  ...
  dplyr::select(id, code = code.low) %>%         ## CJS added dplyr::
  arrange(code)
...
for (i in 1:8){
  list.rw.un[[i]] <- list.rw[[i]] %>%
    ...
    dplyr::select(id:ctry, iv.gp)     ## CJS added dplyr
}
...
ctry.use <- read.csv("CTS_ALL_16012021.csv")  ## CJS updated csf file name
...
for (i in 1:8){
  nm.yr[[i]] <- list.rw[[i]][ , -4] %>%
    group_by(ctry, year) %>%
    summarise(y = n())
  nm.yr.all[[i]] <- expand.grid(ctry = as.character(ctry.name), year = year.crit.from:year.crit.to) %>%  ## CJS year.to to year.crit.to
    left_join(nm.yr[[i]], by  = c("ctry", "year")) %>%
    replace_na(list(y =0))
}

nyr <- year.crit.to - year.crit.from + 1   ## CJS changed year.to to year.crit.to et
...
iv.rte.yr.un.df <- expand.grid(ctry = ctry.name, year = year.crit.from:year.crit.to)  ## CJS year.to to year.crit.to
iv.rte.yr.un.df <- iv.rte.yr.un.df %>%
#  mutate(    ## CJS see changed by DDF
#         log.iv.rte2 = as.numeric(log.iv.rte2.yr.un)
#  )
  log.iv.rte2=log.iv.rte2.yr.un %>%
  mutate_at(vars(log.iv.rte2),numeric)
  
  
# Write to file

setwd(path.data)
file.name <- paste("Trade Route Ctry_Year un disc_", incl.disc, "_", year.crit.from, "_", year.crit.to, ".csv", sep = "")  ## CJS year.to to year.crit.to
