...
#
year.crit.from <- 2008   ## CJS  changed name to year.crit.from
year.crit.to <- 2017     ## CJS  changed name to year.crit.tp
#_______________________________________________________________________
#
...
#
#path.Rcode <- 'C:/ETIS/analysis/R code'         ## CJS  set outside code blocc
#path.data <- 'C:/ETIS/analysis/Processed Data'  ## CJS  set outside code block
# 
#_______________________________________________________________________
#
...
y0 <- max(min(df.1$seizure_year), year.crit.from)    ## CJS changed year.from to year.crit.from
y1 <- min(max(df.1$seizure_year), year.crit.to)      ## CJS changed year.to   to year.crit.to
...
filename <- paste('dfinout_gps_transallop_', ctry_noszopp_included, '_ctrydest_', ctry_dest_included, '_', 
                  year.crit.from, '_', year.crit.to, '_tidy.csv', sep = '')  ## CJS changed year.to to year.crit.to..ditto for year.from
