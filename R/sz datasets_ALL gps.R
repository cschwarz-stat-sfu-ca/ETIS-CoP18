...
#year.crit.from <- 2008   ## CJS set outside code
#year.crit.to <- 2017     ## CJS set outside code
...
file.inout <-  paste('dfinout_gps_transallop_TRUE_ctrydest_TRUE_', year.crit.from, '_', year.crit.to, '_tidy.csv', sep = '') ## CJS added _tidy
df.inout <- read.csv(file.inout, header = TRUE) ## CJS added TRUE
...
df.szrecs <- read.csv(paste('sz recs with estd wgts ', year.from, '_', year.to, '.csv', sep = ''), header = T)
...
#df.raw <- df.szrecs %>%
#  filter(raw) %>%
#  dplyr::select(sz.id = sz.id, sz.yr = sz.yr, ctry = disc.ct,   ## CJS added dplyr::
#          pcs = raw.pcs, wgt = raw.wgt, lo.cl = raw_l, up.cl = raw_u, grp = raw.grp) %>%
#  dplyr::mutate(grp = reorder(grp, new.order = c(1, 2, 3, 5, 4, 6)))

df.raw <- df.szrecs %>%  ## CJS see new code in Daniela Di Filippo directory
  filter(raw) %>%
  dplyr::select(sz.id = sz.id, sz.yr = sz.yr, ctry = disc.ct, ## CJS added plyr::
         pcs = raw.pcs, wgt = raw.wgt, lo.cl = raw_l, up.cl = raw_u, grp = raw.grp) %>%
...
df.wkd <- df.szrecs %>%   ## CJS see new code in Daniela Di Filippo directory
  filter(wkd) %>%
  dplyr::select(sz.id = sz.id, sz.yr = sz.yr, ctry = disc.ct,   ## CJS added dplyr::
         pcs = wkd.pcs, wgt = wkd.wgt, lo.cl = wkd_l, up.cl = wkd_u, grp = wkd.grp) %>%
  mutate(grp = factor(grp))%>%
