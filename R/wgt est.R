...
#path.Rcode <- 'C:/ETIS/analysis/R Code'         ## CJS
#______________________________________________________________________
#
...
df.szrecs <- read.csv(file.path(path.data,file.in), header=TRUE)  # don't use T/F inplace of TRUE/FALSE  ## CJS fixed path
...
names(df.raw)[4:5] <- c('pcs', 'wgt')  ## CJS don't select by column number. Use plyr::rename() function

df.wkd <- df.szrecs %>%
  filter(wkd) %>%
  dplyr::select(sz.id, sz.yr, disc.ct, wkd.pcs, wkd.wgt)  ## CJS  added dplyr::

names(df.wkd)[4:5] <- c('pcs', 'wgt') ## CJS don't select by column numbers. Use plyr::rename() function
...
df.r.pred <- df.raw %>%
  ...
  dplyr::select(sz.id, sz.yr, pcs, wgt, x)  ## CJS added dplyr::  
